...
# of halos in the FLAMINGO-L1_m8 halo catalog.
...
def main():
    ...
    data_path = "<path/to/FLAMINGO/halo/catalogs/>" # path to halo catalogs
    sim_name = "FLAMINGO_L1000N3600" # simulation name that will be used to save .csv files
    snap = int(sys.argv[1]) # snapshot for simulations
    halo_file = data_path + f"FLAMINGO_L1000N3600_halo_catalog_snap00{snap}.h5" # path for FLAMINGO-L1_m8 halos
    
    print("\n\033[1mPreprocessing data...\033[0m")

    ##### Load Halo catalog

    # Load FLAMINGO-L1_m8 halo catalog at snapshot provided above.
    data = load_h5_halo_catalog(halo_file)
