...
These are the scripts used to analyze the IllusrisTNG (TNG-300-1), TNG-Cluster, FLAMINGO-L1_m8 halo catalogues in `halo_catalogs` to produce scaling parameters (normalization, slope, intrinsic scatter), covariances, correlations, and mass-proxy qualities (MPQs) that are saved (after a bit of cleaning) in `Data` directory as `.pkl` files. The results of this analysis were presented in the following publication [Aljamal et. al 2025](https://academic.oup.com/mnras/advance-article/doi/10.1093/mnras/staf1665/8269934).
